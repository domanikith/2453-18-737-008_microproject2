...
{
  const acct_num = agent.parameters.number;
  const client = new MongoClient(url);
  await client.connect();
  const snap = await client.db("CostumerDetails").collection("costumer_details").findOne({acct_num: acct_num});
...
function report_issue(agent)
{
  ...
  var dbo = db.db("issue_details");
	...
	var myobj = { username:u_name, issue:issue_val,status:status,issue_date_time:time_date,trouble_ticket:trouble_ticket };
